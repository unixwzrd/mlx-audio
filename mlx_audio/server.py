...
async def tts_speech(payload: SpeechRequest, request: Request):
    ...
    if payload.ref_text and isinstance(payload.ref_text, str) and os.path.exists(payload.ref_text):
        try:
            with open(os.fspath(payload.ref_text), encoding="utf-8") as fh:
                payload.ref_text = fh.read()
        except OSError as exc:
            raise HTTPException(
                status_code=400,
                detail=f"Reference text file unreadable: {payload.ref_text}: {exc}"
            ) from exc
